...
この機能はCodex所有の技術タスクで使う任意の台帳・レビュー経路です。
Hermesを含む全処理の共通最終監査ゲートではありません。HermesはHermes領域、
CodexはCodex領域を検証し、システム間連携の主導はCodexが担当します。

## まず覚えること

- Hermes内部の処理はHermesが担当し、Codex所有タスクの受付・設計判断はCodexが担当します。
- ローカルPCが起動していて利用可能なら、重い実装をローカルCodexへ送ります。
- ローカルPCが使えなければ、VPS上のCodexへ自動的に切り替えます。
- VPS Codex監査は、明示的に独立レビューを求めたCodex所有タスクだけで使用します。
- 台帳には作業の事実だけを保存し、音声本文、文字起こし本文、APIキーなどは保存しません。

完了条件は所有領域ごとに定義し、Hermesの完了をCodex監査へ一律に依存させません。

HermesからVPS Codexへ依頼を渡すことはできますが、直接の
`codex exec --sandbox danger-full-access` は禁止です。Codex workerが作業場所、
ブランチ、sandbox、単一実装担当を固定してから実行します。
...
音声・文字の依頼
      ↓
Codexが所有境界を判定して技術作業として登録
      ↓
ローカルPCのCodexを確認
      ├─ 利用可能 → local-codexが実装
      └─ 利用不可 → vps-codexが実装
      ↓
Codexが変更点・テスト・稼働確認を記録
      ↓
必要な場合だけCodex領域内で独立レビュー
...
次の操作は所有者を明確にし、利用者の明示承認と所有領域内の検証が必要です。
Codex監査は一律必須ではありません。
...
削除、秘密情報、公開設定、本番データ、デプロイのような重大操作は、
所有領域内の検証に加えて利用者の明示承認が必要です。
